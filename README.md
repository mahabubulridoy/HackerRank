...
* The io module is imported from the standard library to handle input/output operations.  
...
* _num_str_1 and _num_str_2 are mutable String variables used to store user input.  
...
1. io::stdin() opens the standard input stream.  
2. read_line reads a line of input from the user and stores it in the provided String variable. 
3. .ok() ensures that the operation succeeded.  
4. .expect("read error") terminates the program with the message "read error" if an error occurs during input.  
...
1. .trim() removes any leading or trailing whitespace (including newlines) from the input.  
2. .parse() attempts to convert the string to an integer of type i32.  
3. .ok() ensures that the parsing succeeded.  
4. .expect("parse error") terminates the program with the message "parse error" if parsing fails.  
...
1. The addition _num_1 + _num_2 computes the sum of the two integers.  
2. println! prints the result to the console.  
